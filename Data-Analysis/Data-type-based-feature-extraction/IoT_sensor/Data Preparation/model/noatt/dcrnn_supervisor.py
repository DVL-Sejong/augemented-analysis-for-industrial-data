...
from model.noatt.dcrnn_model import DCRNNModel
from model.noatt.loss import masked_mae_loss
...
class DCRNNSupervisor:
    def __init__(self, adj_mx, **kwargs):
        ...
        self._model_path = kwargs.get('model_dir')
    ...
    def save_model(self, epoch, path):
        ...
        config['epoch'] = epoch
        torch.save(config, path + '/epo%d.tar' % epoch)
        self._logger.info("Saved model at {}".format(epoch))
        return path + '/epo%d.tar' % epoch

    def load_model(self):
        self._setup_graph()
        self.model_path = self._train_kwargs.get('model_filename', 0)
        assert os.path.exists(self.model_path+'epo%d.tar' % self._epoch_num), 'Weights at epoch %d not found' % self._epoch_num
        checkpoint = torch.load(self.model_path+'epo%d.tar' % self._epoch_num, map_location='cpu')
    ...
    def _train(self, base_lr,
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            if val_loss < min_val_loss:
                wait = 0
                if save_model:
                    model_file_name = self.save_model(epoch_num, self._model_path)
                    self._logger.info(
