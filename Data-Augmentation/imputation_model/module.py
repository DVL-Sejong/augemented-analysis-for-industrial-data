...
from .epsilon_theta_graph import EpsilonTheta


class GraphTimeGradModel(nn.Module):
    """
    ...
    def __init__(
        ...
        self.memory = nn.Parameter(torch.randn(hidden_size, hidden_size), requires_grad=True)

        # 여기부터 내가 추가
        self.We1 = nn.Parameter(torch.randn(self.rnn_input_size, hidden_size), requires_grad=True)
        self.We2 = nn.Parameter(torch.randn(self.rnn_input_size, hidden_size), requires_grad=True)
        self.Wq = nn.Parameter(torch.randn(hidden_size, hidden_size), requires_grad=True) # 차원 확인해봐야됨
        self.graph_weights = torch.nn.Parameter(torch.FloatTensor(2 * 3 * self.rnn_input_size, self.rnn_input_size))
        # 여기까지
        ...
        self.decoder = EpsilonTheta(target_dim=input_size, cond_dim=hidden_size)
    ...
    def unroll_lagged_rnn(
        ...
        rnn_input_update = self.GCN(rnn_input)
        output, new_state = self.rnn(rnn_input_update)
        h_att = self.att_hidden(output)

        return loc, scale, h_att, static_feat, new_state
    ...
    def GCN(
        ...
        for support in support_set:
            x_g.append(torch.matmul(rnn_input, support))
        x_g = torch.cat(x_g, dim=-1) 
        x_gconv = torch.matmul(x_g, self.graph_weights)  # b, N, dim_out
    ...
    def att_hidden(
        ...
        query = torch.matmul(hidden_state, self.Wq)     # (B, N, d)
        att_score = torch.softmax(torch.matmul(query, self.memory.t()), dim=-1)
        h_att = torch.matmul(att_score, self.memory)
    ...
    def sample(self, context, loc, scale):
        ...
        for t in self.scheduler.timesteps:
            model_output = self.decoder(sample, t, context.view(B * T, 1, -1))
