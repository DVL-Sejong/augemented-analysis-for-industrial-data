...
PALETTE = sns.color_palette(None, 255)
palette_index = 0
for _color_float in PALETTE:
  _c_int_255 = (int(_color_float[0]*255), int(_color_float[1]*255), int(_color_float[2]*255), 255)
...
def draw_circle():
  global IMAGE
  if len(AREAS) != 0 and VISIBLE_FLAG:
    colorIdx = 0
    for area in AREAS:
      count = 0
      prevPt = AREAS[0]
...
while True:
  ...
  if key == 27:
    ...
    print("append area started: "+str(len(AREAS)+1))
    IMAGE = cv2.imread(sti_path)
    if len(CLICKS) < 3:
      continue
    area = []
    ...
    AREAS.append(area)
    print("append area done: "+str(len(AREAS)+1))
    CLICKS = []
    if mouse_down_flag == True:
      mouse_down_flag = False
    continue
  elif key == ord('n'):
    print(STI_LIST[STIMULUS_INDEX])
    
    # segmentation data save process
    img_height, img_width = IMAGE.shape[:2]
    areaCount = 1
    allPixels = np.zeros(IMAGE.shape[:2])
    print('segmentation matrix generation process started.')
    for area in AREAS:
      print("----"+str(areaCount)+"/"+str(len(AREAS)))
