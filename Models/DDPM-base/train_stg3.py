...
from algorithm.diffstg3.model import DiffSTG, save2file
...
def evals(model, data_loader, epoch, metric, config, clean_data, mode='Test'):
    ...
    for i, batch in enumerate(data_loader):
        ...
        message = f"{i / len(data_loader)} % done"
        print()
...
def main():
    ...
    for epoch in range(config.epoch):
        ...
        n, avg_loss, avg_loss1, avg_loss2, avg_loss3, time_lst = 0, 0,0,0,0, []
        ...
        for i, batch in enumerate(train_loader):
            ...
            loss1, loss2, loss3 = model.loss(x, (x_masked, pos_w.to(config.device), pos_d.to(config.device)))

            loss = loss1 * 10 + loss2 *0.1 + loss3 *0.1
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            avg_loss1 = avg_loss1 * (n - 1) / n + loss1.item() / n
            avg_loss2 = avg_loss2 * (n - 1) / n + loss2.item() / n
            avg_loss3 = avg_loss3 * (n - 1) / n + loss3.item() / n

            time_lst.append((timer() - time_start))
            message = f"{i / len(train_loader) + epoch:6.1f}| {avg_loss:0.3f} loss1:{avg_loss1:0.3f} loss2:{avg_loss2:0.3f} loss3:{avg_loss3:0.3f} {np.sum(time_lst):.1f}s"
            print('\r' + message, end='', flush=True)
